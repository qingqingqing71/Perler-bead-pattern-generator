...
export default function Home() {
  ...
  const handleGenerateBeadPattern = useCallback(async () => {
    ...
    try {
      // 所有采样模式统一使用 generateBeadPatternHD 函数
      // 该函数已经重写为与 generateBeadPatternV2 完全一致的处理逻辑
  ...
  const handleDownloadBeadPattern = useCallback(async () => {
    ...
    try {
      // 所有采样模式统一使用 generateBeadPatternHD 函数
      const result = await generateBeadPatternHD(pixelatedSubject, effectiveGridCols, effectiveGridRows, 3, true, colorMatchAccuracy, samplingMode, beadColors);
                ...
                <div className="mt-6 space-y-3">
                  ...
                  <div className="flex gap-3">
                    ...
                    <Button
                      variant="outline"
                      onClick={handleReset}
                      className={beadPatternImage ? "" : "flex-1"}
                    >
                      重新开始
                    </Button>
                  </div>
