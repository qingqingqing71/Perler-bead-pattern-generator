...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await client.generate({
      prompt: 'Transform this image into semi-realistic cartoon anime art style. STYLE SPECIFICATIONS: 1) REALISTIC PROPORTIONS - Keep normal human body proportions, NOT chibi or super-deformed, natural head-to-body ratio 2) SOFT PASTEL COLORS - Fresh, gentle color palette with clear color areas, minimal gradients, harmonious tones 3) EXPRESSIVE ANIME EYES - Moderately sized anime-style eyes with highlights and depth, realistic nose shape, natural mouth, maintain original facial expression 4) CLEAN ANIME OUTLINES - Smooth defined edges, clean line art style, neat color separation 5) KEY FEATURES PRESERVED - Same hairstyle, same clothing, same accessories, same iconic items, same pose and body orientation 6) FACE DETAILS - Keep identical expression, identical eye direction, identical mouth shape, realistic facial structure 7) ACCESSORIES MATCH - Glasses, hats, earrings in EXACT same positions with same shapes 8) WHITE AREAS STAY WHITE - White clothes/hair/accessories remain solid white pixels, NOT transparent 9) TRANSPARENT BACKGROUND - Only areas outside character silhouette should be transparent 10) CLEAN FLAT STYLE - Simple shading, soft shadows, flat color aesthetic, modern anime illustration style 11) NO extra elements - No watermark, text, signature, border, background fill, or canvas extension. Output only the character tightly cropped.',
      image: imageUrl,
      size: '2K',
    });
